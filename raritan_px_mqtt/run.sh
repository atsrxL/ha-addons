...
export INLET_REAL_POWER_CALIBRATION="$(bashio::config 'inlet_real_power_calibration' 'false')"
export INLET_FIXED_OFFSET_CALIBRATION="$(bashio::config 'inlet_fixed_offset_calibration' 'false')"
export HIDE_OUTLET_SENSORS="$(python3 -c 'import json; data=json.load(open("/data/options.json", encoding="utf-8")); print(json.dumps(data.get("hide_outlet_sensors", []), separators=(",", ":")))')"
